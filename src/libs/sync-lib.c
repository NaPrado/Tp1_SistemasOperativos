...
void set_init_semaphores(Tgame_sync * game_sync) {
    ...
    if (semctl(sem_id, SHOW_NEEDED, SETVAL, 0) == -1) {
        perror("semctl SHOW_NEEDED failed");
        exit(EXIT_FAILURE);
    }
    if (semctl(sem_id, SHOW_DONE, SETVAL, 0) == -1) {
        perror("semctl SHOW_DONE failed");
        exit(EXIT_FAILURE);
    }
    if (semctl(sem_id, MASTER_MUTEX, SETVAL, 1) == -1) {
        perror("semctl MASTER_MUTEX failed");
        exit(EXIT_FAILURE);
    }
    if (semctl(sem_id, GAME_STATE_MUTEX, SETVAL, 1) == -1) {
        perror("semctl GAME_STATE_MUTEX failed");
        exit(EXIT_FAILURE);
    }
    if (semctl(sem_id, PLAYER_READ_COUNT_MUTEX, SETVAL, 1) == -1) {
...
void set_player_reading(Tgame_sync * game_sync) {
    ...
    if (game_sync->player_reading_status++ == 0) {
        ...
        if (semop_retry(sem_id, ops2, 1) == -1) {
